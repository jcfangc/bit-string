...
fn returns_low_bits_for_small_widths() {
    ...
    for (bits, expected) in cases {
        assert_eq!(<[u64]>::low_mask(bits), expected, "bits={bits}");
    }
}

#[test]
fn returns_all_ones_for_full_word_width() {
    assert_eq!(<[u64]>::low_mask(WORD_BITS), u64::MAX);
}

#[test]
fn clears_only_the_top_bit_for_word_bits_minus_one() {
    assert_eq!(<[u64]>::low_mask(WORD_BITS - 1), u64::MAX >> 1);
}
...
fn clamps_to_max_for_excess_bits() {
    assert_eq!(<[u64]>::low_mask(WORD_BITS + 1), u64::MAX);
    assert_eq!(<[u64]>::low_mask(128), u64::MAX);
    assert_eq!(<[u64]>::low_mask(usize::MAX), u64::MAX);
}
