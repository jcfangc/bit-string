...
mod neon {
    ...
    pub(super) unsafe fn eq_words(sw: &[u64], pw: &[u64], len: usize) -> bool {
        let mut i = 0;
        while i + 2 <= len {
            let a: uint64x2_t = unsafe { vld1q_u64(sw.as_ptr().add(i)) };
            let b: uint64x2_t = unsafe { vld1q_u64(pw.as_ptr().add(i)) };
