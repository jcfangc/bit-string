...
fn empty_needle_matches_at_zero() {
    ...
    assert_eq!(bits.find_bits(&needle), Some(0));
}
...
fn returns_none_when_needle_is_longer() {
    ...
    assert_eq!(bits.find_bits(&needle), None);
}
...
fn finds_needle_in_middle() {
    ...
    assert_eq!(bits.find_bits(&needle), Some(2));
}
...
fn finds_needle_at_end() {
    ...
    assert_eq!(bits.find_bits(&needle), Some(3));
}
...
fn finds_match_across_word_boundary() {
    ...
    assert_eq!(bits.find_bits(&needle), Some(63));
}
