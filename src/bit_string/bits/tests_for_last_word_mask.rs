...
fn returns_full_mask_when_len_is_word_aligned() {
    assert_eq!(Bits::last_word_mask(0), u64::MAX);
    assert_eq!(Bits::last_word_mask(WORD_BITS), u64::MAX);
    assert_eq!(Bits::last_word_mask(WORD_BITS * 2), u64::MAX);
}
...
fn keeps_exact_low_bits_for_partial_last_word() {
    ...
    for (len, expected) in cases {
        assert_eq!(Bits::last_word_mask(len), expected, "len={len}");
    }
}
...
fn mask_has_no_high_bits_above_remainder() {
    ...
    for len in lens {
        ...
        let mask = Bits::last_word_mask(len);
