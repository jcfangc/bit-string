...
fn returns_word_when_bit_start_is_word_aligned() {
    ...
    assert_eq!(Bits::read_a_word_at(&src, 0), src[0]);
    assert_eq!(Bits::read_a_word_at(&src, WORD_BITS), src[1]);
}
...
fn returns_zero_when_aligned_start_is_past_source() {
    ...
    assert_eq!(Bits::read_a_word_at(&src, WORD_BITS), 0);
    assert_eq!(Bits::read_a_word_at(&src, WORD_BITS * 2), 0);
}
...
fn reads_shifted_chunk_within_single_word() {
    ...
    assert_eq!(Bits::read_a_word_at(&src, 2), 0b10_1101);
    assert_eq!(Bits::read_a_word_at(&src, 4), 0b1011);
}
...
fn reads_chunk_across_word_boundary() {
    ...
    assert_eq!(Bits::read_a_word_at(&src, WORD_BITS - 1), 0b10101);
}
...
fn treats_missing_high_word_as_zero() {
    ...
    assert_eq!(Bits::read_a_word_at(&src, WORD_BITS - 1), 1);
}
...
fn preserves_full_chunk_layout_across_boundary() {
    ...
    assert_eq!(Bits::read_a_word_at(&src, shift), expected);
}
...
fn reads_from_later_word_with_unaligned_start() {
    ...
    assert_eq!(Bits::read_a_word_at(&src, WORD_BITS + 4), expected);
}
