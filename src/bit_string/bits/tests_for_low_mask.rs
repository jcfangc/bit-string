...
fn returns_low_bits_for_small_widths() {
    ...
    for (bits, expected) in cases {
        assert_eq!(Bits::low_mask(bits), expected, "bits={bits}");
    }
}

#[test]
fn returns_all_ones_for_full_word_width() {
    assert_eq!(Bits::low_mask(WORD_BITS), u64::MAX);
}

#[test]
fn clears_only_the_top_bit_for_word_bits_minus_one() {
    assert_eq!(Bits::low_mask(WORD_BITS - 1), u64::MAX >> 1);
}
