...
fn collect_bits(words: &[u64], len: usize) -> Vec<bool> {
    (0..len).map(|index| bit_at(words, index)).collect()
}
...
fn copies_aligned_full_word_into_empty_destination() {
    ...
    copy_bits(&src, 0, &mut dst, 0, WORD_BITS);
...
fn copies_across_source_word_boundary() {
    ...
    copy_bits(&src, WORD_BITS - 2, &mut dst, 0, 6);
...
fn copies_across_destination_word_boundary() {
    ...
    copy_bits(&src, 0, &mut dst, WORD_BITS - 2, 6);

    assert_eq!(bit_at(&dst, WORD_BITS - 2), true);
    assert_eq!(bit_at(&dst, WORD_BITS - 1), false);
    assert_eq!(bit_at(&dst, WORD_BITS), true);
    assert_eq!(bit_at(&dst, WORD_BITS + 1), false);
    assert_eq!(bit_at(&dst, WORD_BITS + 2), false);
    assert_eq!(bit_at(&dst, WORD_BITS + 3), true);
}
...
fn leaves_bits_outside_destination_range_unchanged() {
    ...
    assert_eq!(bit_at(&dst, 1), true);
    assert_eq!(bit_at(&dst, 20), true);

    assert_eq!(bit_at(&dst, 4), true);
    assert_eq!(bit_at(&dst, 5), false);
    assert_eq!(bit_at(&dst, 6), true);
    assert_eq!(bit_at(&dst, 7), false);
    assert_eq!(bit_at(&dst, 8), false);
    assert_eq!(bit_at(&dst, 9), true);
}
...
fn copies_more_than_one_chunk() {
    ...
    copy_bits(&src, 0, &mut dst, 0, WORD_BITS * 2 + 2);

    for index in 0..(WORD_BITS * 2 + 2) {
        assert_eq!(bit_at(&dst, index), bit_at(&src, index), "index={index}");
    }
}
