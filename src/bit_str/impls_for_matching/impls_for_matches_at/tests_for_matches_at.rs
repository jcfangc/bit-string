...
fn matches_at_exact_match() {
    ...
    assert!(v.matches_at_str(0, p.as_bit_str()));
    assert!(!v.matches_at_str(1, p.as_bit_str()));
}
...
fn matches_at_beyond_view_returns_false() {
    ...
    let p = BitString::try_from("10").unwrap();
    assert!(!v.matches_at_str(2, p.as_bit_str()));
}
...
fn matches_at_empty_pattern_always_true() {
    ...
    let p = BitString::new();
    assert!(v.matches_at_str(0, p.as_bit_str()));
    assert!(v.matches_at_str(3, p.as_bit_str()));
}
