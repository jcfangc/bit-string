use bit_string::BitString;
use divan::{Bencher, black_box};

fn main() {
    divan::main();
}

// Truncate 65→64 crosses a word boundary, testing the lazy-shrink fast path.
#[divan::bench(name = "truncate/65_to_64/bit_string")]
fn truncate_65_to_64_bit_string(bencher: Bencher) {
    bench_bit_string(bencher, 65, 64);
}

#[divan::bench(name = "truncate/65_to_64/string")]
fn truncate_65_to_64_string(bencher: Bencher) {
    bench_string(bencher, 65, 64);
}

// Truncate 65536→128 removes many words, tests batch-shrink behavior.
#[divan::bench(name = "truncate/65536_to_128/bit_string")]
fn truncate_65536_to_128_bit_string(bencher: Bencher) {
    bench_bit_string(bencher, 65_536, 128);
}

#[divan::bench(name = "truncate/65536_to_128/string")]
fn truncate_65536_to_128_string(bencher: Bencher) {
    bench_string(bencher, 65_536, 128);
}

// Truncate 128→65 partial-word rewrite in the last word.
#[divan::bench(name = "truncate/128_to_65/bit_string")]
fn truncate_128_to_65_bit_string(bencher: Bencher) {
    bench_bit_string(bencher, 128, 65);
}

#[divan::bench(name = "truncate/128_to_65/string")]
fn truncate_128_to_65_string(bencher: Bencher) {
    bench_string(bencher, 128, 65);
}

fn bench_bit_string(bencher: Bencher, len: usize, target: usize) {
    let input = make_bit_string(len);

    bencher.bench(|| {
        let mut bits = black_box(input.clone());
        bits.truncate(target);
        black_box(bits)
    });
}

fn bench_string(bencher: Bencher, len: usize, target: usize) {
    let input = make_string(len);

    bencher.bench(|| {
        let mut s = black_box(input.clone());
        s.truncate(target);
        black_box(s)
    });
}

#[inline]
fn make_bit_string(len: usize) -> BitString {
    (0..len).map(|index| bit_at(index)).collect()
}

fn make_string(len: usize) -> String {
    let mut out = String::with_capacity(len);
    for index in 0..len {
        out.push(bit_char(bit_at(index)));
    }
    out
}

#[inline]
fn bit_char(value: bool) -> char {
    if value { '1' } else { '0' }
}

#[inline]
fn bit_at(index: usize) -> bool {
    mix64(index as u64) & 1 != 0
}

#[inline]
fn mix64(mut value: u64) -> u64 {
    value = value.wrapping_add(0x9e37_79b9_7f4a_7c15);
    value = (value ^ (value >> 30)).wrapping_mul(0xbf58_476d_1ce4_e5b9);
    value = (value ^ (value >> 27)).wrapping_mul(0x94d0_49bb_1331_11eb);
    value ^ (value >> 31)
}
